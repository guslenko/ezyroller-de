<template>
  <div class="relative group">
    ...
    <button
      type="button"
      class="inline-flex items-center justify-center px-2 py-2 gap-2 bg-white rounded-md text-primary-700 hover:bg-slate-200 active:bg-slate-300"
    >
    ...
    <!-- Выпадающее меню -->
    <div
      class="absolute right-0 top-full mt-2 bg-white border border-slate-200 shadow-lg rounded-md opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-150 z-50 min-w-[160px]"
    >
      <ul class="py-2">
        <li
        ...
        >
          ...
          <SfIconCheck v-if="localeCode === currentLocale" class="text-green-500 ml-auto" />
        </li>
      </ul>
    </div>
  </div>
