    ...
    <div class="px-4 md:px-0">
      ...
      >
        ...
        <div v-if="columns[0]" class="relative" data-testid="multi-grid-column">
          <div v-for="row in columns[0]" :key="row.meta.uuid" class="relative group" :data-uuid="row.meta.uuid">
            <slot name="content" :content-block="row" :column-length="columns[0].length" :is-row-hovered="false" />
          </div>
        </div>

        <!-- CONTENT -->
        <div v-if="columns[1]" class="relative" data-testid="multi-grid-column">
          <div v-for="row in columns[1]" :key="row.meta.uuid" class="relative group" :data-uuid="row.meta.uuid">
            <slot name="content" :content-block="row" :column-length="columns[1].length" :is-row-hovered="false" />
          </div>
        </div>
      </div>
    </div>
