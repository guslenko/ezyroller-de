...
import type { SubmitEzyrollerCancellation, UseEzyrollerCancellationFormState } from './types';

export const useEzyrollerCancellationForm = () => {
  const state = useState<UseEzyrollerCancellationFormState>('useCancellationForm', () => ({
    loading: false,
  }));
  ...
  const submitCancellation: SubmitEzyrollerCancellation = async (params) => {
